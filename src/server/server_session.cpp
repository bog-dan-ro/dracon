...
basic_server_session::basic_server_session(dracon::internal::sessions_event_loop *event_loop, int sock, std::string peer_addr, uint32_t order)
    : m_sock(sock)
    , m_order(order)
...
const std::string &basic_server_session::peer_address() const noexcept
{
    return m_peer_addr;
}
